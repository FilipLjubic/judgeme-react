...
The first thirteen components are `StarRatingBadge`, `AllReviewsCounter`, `ReviewsCarousel`, `LegacyReviewWidget`, `AllReviewsWidget`, `FloatingReviewsTab`, `ReviewsGrid`, `CardsCarousel`, `TestimonialsCarousel`, `VideosCarousel`, `PopupReviews`, `AiReviewsSummary`, and `ReviewSnippets`. The legacy components share Judge.me's dashboard CSS/settings and browser runtime; the exact extension adapters combine current public data or Shopify metafields with the store's deployment scripts. The Hydrogen harness exercises them with real review data plus a clearly labeled AI-summary fixture, including all five carousel/snippet implementations, the write-review modal, standalone All Reviews streams, the floating tab, grid pagination, the timed popup, the AI summary accordion, and the v3 media lightboxes.
