...
- Treat `JUDGE_ME_WIDGETS` as a coverage roadmap, not an implementation registry. At present only `StarRatingBadge`, `ReviewsCarousel`, `LegacyReviewWidget`, `AllReviewsWidget`, and `FloatingReviewsTab` are implemented. Do not describe the catalog as supported until each entry has a data adapter, public component, tests/fixtures, and browser verification.
...
- When the same route also renders shop widgets, use `fetchLegacyStorefrontWidgets`. It adds `featured_carousel`, `all_reviews_page`, and `reviews_tab` while keeping one shared settings/CSS payload. The classic carousel, All Reviews Widget, and floating tab are shop-level and must not receive a product `external_id`. Reuse the one `all_reviews_page` response for the Free-plan Floating Tab fallback.
...
- Judge.me's secondary bundle closes over the Floating Reviews Tab roots present on its first document-ready pass. A remounted SPA root therefore needs React-owned open/close and Escape listeners. Keep those listeners component-scoped, release the body scroll lock on unmount, and intercept fallback controls before Judge.me tries the unavailable `reviews_tab` endpoint.
